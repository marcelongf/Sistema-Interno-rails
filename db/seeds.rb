...
5.times do |c|
    Directorship.create([{name: Faker::Movies::LordOfTheRings.location}, {director: 1 + c}])
end

5.times do |manager_id|
    5.times do |dir_id|
        Cell.create([{name: Faker::Movies::HarryPotter.location}, {manager: manager_id}, {directorship_id: dir_id}])
    end
end

User.create([{email: 'admin@email.com'}, {name: Faker::Games::LeagueOfLegends.champion}, {encrypted_password: '1234567'}, {cell_id: 1}, {directorship_id: 1}, {kind: 'gp_director'}])
